...
trait FiltersByPopularityTimeframe
{
    ...
    {
        $query->popularBetween($startOfLastWeek = now()->subDays(7)->startOfWeek(), $startOfLastWeek->copy()->endOfWeek());
    }

    public function scopePopularLastMonth(Builder $query)
    {
        $query->popularBetween(now()->startOfMonth()->subMonthWithNoOverflow(), now()->subMonthWithNoOverflow()->endOfMonth());
    }
    ...
    public function scopePopularBetween(Builder $query, Carbon $from, Carbon $to)
    {
        $query->whereHas('visits', $this->betweenScope($from, $to))
            ->withCount(['visits as visit_count' => $this->betweenScope($from, $to)]);
    }
    ...
    {
        $query->popularBetween(now()->startOfYear()->subYearWithoutOverflow(), now()->subYearWithoutOverflow()->endOfYear());
    }
